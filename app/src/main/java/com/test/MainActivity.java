...
public class MainActivity extends AppCompatActivity {
     ...
     * 5.关键字判断模拟器.microvirtd是逍遥模拟器/genyd是genymotion模拟器
     * gps逍遥模拟器
     * ro.microvirtd.caps.gps=on ?
     * genymition模拟器
    ...
    private void getEmuInfo() {
        try {
             ...
             * 蓝叠(Bluestack2)模拟器 ro.product.name kletxx  ro.product.brand/ro.board.platform msm8974
             */
            ...
            sb.append("\n==========制造商==============\n")
                    .append("制造商[manufacturer]:").append(Build.MANUFACTURER).append("\n")
                    .append("制造商[runtime获取]:").append(shell("getprop ro.product.manufacturer")).append("\n")
                    .append("制造商[build.prop获取]:").append(prop.get("ro.product.manufacturer")).append("\n")
                    .append("\n========网络(有线/无线)=========\n")
                    .append("wifi[runtime获取]:").append(shell("getprop wifi.interface")).append("\n")
                    .append("wifi[build.prop获取]:").append(prop.get("wifi.interface")).append("\n")
                    .append("\n===========ADB状态=============\n")
                    ...
                    .append("persist.service.adb.enable[shell获取]:").append(shell("getprop persist.service.adb.enable")).append("\n")
                    .append("persist.service.adb.enable[build.prop]:").append(prop.get("persist.service.adb.enable")).append("\n")
                    .append("ro.adb.secure[shell获取]:").append(shell("getprop ro.adb.secure")).append("\n")
                    .append("ro.adb.secure[build.prop]:").append(prop.get("ro.adb.secure")).append("\n")
                    .append("\n===========CPU情况=============\n")
                    .append("cpu[CPU_ABI]:").append(Build.CPU_ABI).append("\n")
                    .append("cpu[runtime获取]:").append(shell("getprop ro.product.cpu.abi")).append("\n")
...
//                    cpuInfo = in.readLine();
    ...
    /**
     * 获取NFC信息.
     * 需要权限:<uses-permission android:name="android.permission.NFC"/>
     * 限制最低版本:14
     * 限制安装硬件,要求当前设备必须要有NFC芯片:<uses-feature  android:name="android.hardware.nfc"  android:required="true" />
     */
